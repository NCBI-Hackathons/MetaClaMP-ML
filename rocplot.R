...
files <- list.files(path = path, pattern = "actual_pred", full.names = T) #file from the svm-sklearn output
...
for (i in 1:length(files)){
	...
	lv=paste0('AUC: ', signif(ci.auc(dfx$X1,dfx$X2)[3],2),' (',signif(ci.auc(dfx$X1,dfx$X2)[3],1),'-',signif(ci.auc(dfx$X1,dfx$X2)[3],2),')') #from pROC #caution
